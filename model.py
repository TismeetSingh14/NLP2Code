...
class Model(nn.Module):
    def __init__(self, pretrained_weights, args):
        ...
        self.decoder = MyTransformerDecoder(
            ...
            norm = LayerNorm(config.hidden_size))
    ...
    def encode(self, source, no_context_update):
        ...
        if self.args.extra_encoder:
            encoder_output = self.extra_encoder(
                src = torch.transpose(encoder_output, 0, 1),
                mask = None, 
    ...
    def forward(self, data, target_input = None, no_encoder = None, no_context_update = False, return_encoder_output = False, encoder_output_saved = None):
        ...
        if self.args.pointer_network:
            choices = {key:value.to(self.device) for key, value in data['choice'].items()}
        ...
        if encoder_output_saved is not None:
            ...
            else: encoder_output = self.encode(source, no_context_update)
        ...
        prediction = self.decoder(
            tgt = torch.transpose(target_embedding, 0 , 1),
        ...
        if self.args.pointer_network:
            ...
            logits = logits.masked_fill_(
                (choices['attention_maks'] == 0).unsqueeze(1).expand(-1, logits.shape[1], -1), float('-inf')
            )
        ...
        if not self.args.no_encoder and self.args.use_copy_attention:
            ...
            logits.scatter_add_(
                index = index.unsqueeze(1).expand(-1, logits.shape[1], -1),
                src = copy_attention, 
                dim = 2
            )
...
class MyEmbedding(nn.Module):
    ...
    def forward(self, input_ids = None, position_ids = None, input_embeds = None):
        ...
        if position_ids is None:
            position_ids = self.pembeddings.position_ids[:,:seq_length]
        
        if input_embeds is None:
            input_embeds = self.pembeddings.word_embeddings(input_ids)
        position_embeddings = self.pembeddings.position_embeddings(position_ids)
